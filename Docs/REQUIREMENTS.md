...
### グラフィックスタイル
- **方向性**: ローポリゴン風 + リッチな発光演出
- **プレイヤー/敵シェーディング**: Three.js `MeshStandardMaterial`をベースにカスタムトゥーンシェーダー（2-3段階グラデーション + リムライト）を適用し、エッジの立ったシルエットと金属感を両立させる。攻撃時はBloomポストエフェクトで発光ラインを強調。
- **障害物/地形**: Triplanarマッピング + ノイズテクスチャでローポリ感を保ちつつ質感を演出。地面・天井はUVオフセットでスクロールし、柱/板には透過っぽいシェーダーで光が滲むような効果を付与。
- **ボス**: SkinnedMeshやShaderMaterialでセグメントの伸縮・回転を制御し、FlowMapベースのエネルギーパターンを流す。攻撃時はディゾルブ表現や追加Bloomで演出強化。
- **影/ポストエフェクト**: PCFSoftShadowMapに加え、軽量SSAO・Chromatic Aberration・フィルミックトーンマッピングを薄く適用してリッチ感を底上げ。
